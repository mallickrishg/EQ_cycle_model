...
% % add time of perturbation
% tperturb = 50.*3.15e7;
% trecover_st = 418.5.*3.15e7;
% trecover_end = 440*3.15e7;
% % stress perturbation
% negstep = 3;
% deltau = 0;
% delsigma = (negstep); % effect of positive delsigma == negative deltau
% dsigma_dt = zeros(rcv.N,1);
% dsigma_dt(top:bottom) = -negstep./(80*3.15e7);
% % total duration of simulation
% tend = 500*3.15e7;
...
caxis([0 2])
hold on
plot([sse_st sse_st],get(gca,'YLim'),'w-')
plot([sse_end sse_end],get(gca,'YLim'),'w-')

box on, grid on
xlabel('Time (yrs)'), ylabel('\zeta_d (km)')
set(gca,'FontSize',12,'YDir','reverse')
xlim([ti-30 tf+2])
print('results/SSE_V_timeseries','-djpeg','-r200')
...
subplot(2,1,2)
plot(t(1:end-1)./3.15e7,mean(V(:,top:bottom),2)./rcv.Vpl(1),'k-','LineWidth',2)
axis tight, grid on
ylim([0 3.5])
xlim([ti-30 tf+2])
set(gca,'YScale','linear')
ylabel('$\frac{V}{V_{pl}}$','Interpreter','latex','FontSize',15)
xlabel('Time (yrs)')

print('results/SSE_meanV_timeseries','-djpeg','-r200')
